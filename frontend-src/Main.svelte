<script>
  ...
  const fetch = ({ reset = false } = {}) => {
    ...
    return (fetching = getNotes({ age, search: searchQuery, page })
      .then((data) => {
  ...
  const fetchFromScratchWithDelay = () => {
...
<section class="uk-flex uk-grid-collapse">
  <aside class="uk-width-1-4 uk-padding-small">
    ...
    <p class="uk-search uk-search-default uk-width-1-1">
      ...
      <input
        bind:value={search}
        on:keyup={fetchFromScratchWithDelay}
    ...
    {#each entries as entry}
      <NoteCard {entry} isActive={entry.id === activeNoteId} searchQuery={search.trim()} />
    {/each}
